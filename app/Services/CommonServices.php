...
use App\Models\ClinicalRequest;
...

    {
        ...
        switch ($tbl) {
            ...
            case 'clinicalRequest':
                $field = $request->field;
                $pathField = "{$field}_realfile";
                $nameField = "{$field}_filename";
                $downloadField = "{$field}_download";

                $boardReplyFile = ClinicalRequest::findOrFail($sid);
                $boardReplyFile->increment($downloadField);

                $this->data = ['realfile' => $boardReplyFile->{$pathField}, 'filename' => $boardReplyFile->{$nameField}];
                break;

            case 'eventFile':
                $eventFile = EventFile::findOrFail($sid);
                $eventFile->increment('download');

                $this->data = ['realfile' => $eventFile->realfile, 'filename' => $eventFile->filename];
                break;
    ...
    public function zipDownloadService(Request $request)
    {
        ...
        switch ($tbl) {
            ...
            case 'event':
                ...
                $event->files()->increment('download');

                $this->data = $this->makeZip("{$event->title}.zip", $event->files, $request->password ?? null);
                break;
