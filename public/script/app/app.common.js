...
const pwdREGEX = (pwd) => {
    ...
    if (pwd.length < 8 || pwd.length > 16) {
        result.msg = '비밀번호는 8자~16로 입력해주세요.';
        return result;
    }

    const hasDigit = /[0-9]/.test(pwd); // 숫자
    const hasLower = /[a-z]/.test(pwd); // 영문 소문자
    const hasUpper = /[A-Z]/.test(pwd); // 영문 대문자
    const hasSpecial = /[^A-Za-z0-9]/.test(pwd); // 특수문자

    // 조건을 만족한 유형의 개수 합산
    const typeCount = [hasLower, hasUpper, hasDigit, hasSpecial].filter(Boolean).length;

    if (typeCount < 2) {
        result.msg = '비밀번호는 영문 소문자/대문자/숫자/특수문자 중 2개 이상의 문자로 입력해주세요.';
        return result;
    }
...
//Email 형
$(document).on("change","input[emailOnly]", function() {
